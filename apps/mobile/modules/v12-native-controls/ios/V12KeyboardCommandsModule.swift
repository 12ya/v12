...
public final class V12KeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("V12KeyboardCommands")

    View(V12KeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: V12KeyboardCommandsView, commands: [String]) in
...
public final class V12KeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.v12FirstResponder == nil else { return }
...
private extension UIView {
  var v12FirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.v12FirstResponder { return responder }
    }
    return nil
  }
}
