...
#import "V12CodeMarkdownTextShadowNode.h"
#import "V12CodeMarkdownTextComponentDescriptor.h"
...
static void V12CodeMarkdownTextApplyParagraphStyles(
...
static void V12CodeMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
...
@protocol V12CodeMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface V12CodeMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<V12CodeMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<V12CodeMarkdownOutsideTapTarget>)target;

@end

static const void *V12CodeMarkdownOutsideTapCoordinatorKey =
    &V12CodeMarkdownOutsideTapCoordinatorKey;

@implementation V12CodeMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<V12CodeMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<V12CodeMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, V12CodeMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<V12CodeMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static V12CodeMarkdownOutsideTapCoordinator *
V12CodeMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  V12CodeMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, V12CodeMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[V12CodeMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        V12CodeMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface V12CodeMarkdownText () <RCTV12CodeMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface V12CodeMarkdownText () <V12CodeMarkdownOutsideTapTarget>
@end

@implementation V12CodeMarkdownText {
  UIView * _view;
  UITextView * _textView;
  V12CodeMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<V12CodeMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const V12CodeMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    V12CodeMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, V12CodeMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [V12CodeMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<V12CodeMarkdownTextProps const>(_props);
  ...
  V12CodeMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  V12CodeMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::V12CodeMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::V12CodeMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<V12CodeMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<V12CodeMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<V12CodeMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == V12CodeMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == V12CodeMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == V12CodeMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == V12CodeMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const V12CodeMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (V12CodeMarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[V12CodeMarkdownTextRun class]]) {
      continue;
    }

    V12CodeMarkdownTextRun* textChild = (V12CodeMarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::V12CodeMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::V12CodeMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> V12CodeMarkdownTextCls(void)
{
  return V12CodeMarkdownText.class;
}
