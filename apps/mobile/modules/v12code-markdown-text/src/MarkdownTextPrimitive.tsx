...
import V12CodeMarkdownTextRunNativeComponent from "./V12CodeMarkdownTextRunNativeComponent";
import V12CodeMarkdownTextNativeComponent from "./V12CodeMarkdownTextNativeComponent";
...
function MarkdownTextPrimitiveChild({ style, children, ...rest }: MarkdownTextPrimitiveProps) {
  ...
  const nativeChildren = React.Children.toArray(children).map((child) => {
    ...
    return (
      ...
      <V12CodeMarkdownTextRunNativeComponent
  ...
  if (!isAncestor) {
    return (
      <TextAncestorContext.Provider value={contextValue}>
        <V12CodeMarkdownTextNativeComponent
          {...textDefaults}
          {...rest}
