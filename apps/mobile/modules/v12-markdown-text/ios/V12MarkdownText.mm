...
#import "V12MarkdownTextShadowNode.h"
#import "V12MarkdownTextComponentDescriptor.h"
...
static void V12MarkdownTextApplyParagraphStyles(
...
static void V12MarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
...
@protocol V12MarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface V12MarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<V12MarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<V12MarkdownOutsideTapTarget>)target;

@end

static const void *V12MarkdownOutsideTapCoordinatorKey =
    &V12MarkdownOutsideTapCoordinatorKey;

@implementation V12MarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<V12MarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<V12MarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, V12MarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<V12MarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static V12MarkdownOutsideTapCoordinator *
V12MarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  V12MarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, V12MarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[V12MarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        V12MarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface V12MarkdownText () <RCTV12MarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface V12MarkdownText () <V12MarkdownOutsideTapTarget>
@end

@implementation V12MarkdownText {
  UIView * _view;
  UITextView * _textView;
  V12MarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<V12MarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const V12MarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    V12MarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, V12MarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [V12MarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<V12MarkdownTextProps const>(_props);
  ...
  V12MarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  V12MarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::V12MarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::V12MarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<V12MarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<V12MarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<V12MarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == V12MarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == V12MarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == V12MarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == V12MarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const V12MarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (V12MarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[V12MarkdownTextRun class]]) {
      continue;
    }

    V12MarkdownTextRun* textChild = (V12MarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::V12MarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::V12MarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> V12MarkdownTextCls(void)
{
  return V12MarkdownText.class;
}
