...
import V12MarkdownTextRunNativeComponent from "./V12MarkdownTextRunNativeComponent";
import V12MarkdownTextNativeComponent from "./V12MarkdownTextNativeComponent";
...
function MarkdownTextPrimitiveChild({ style, children, ...rest }: MarkdownTextPrimitiveProps) {
  ...
  const nativeChildren = React.Children.toArray(children).map((child) => {
    ...
    return (
      ...
      <V12MarkdownTextRunNativeComponent
  ...
  if (!isAncestor) {
    return (
      <TextAncestorContext.Provider value={contextValue}>
        <V12MarkdownTextNativeComponent
          {...textDefaults}
          {...rest}
