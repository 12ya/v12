...
describe("DesktopClerk", () => {
  ...
  it("derives the Clerk Frontend API hostname used by the desktop CSP", () => {
    const publishableKey = `pk_test_${btoa("clerk.v12code.codes$")}`;
        ...
        [
          {
            ...
            passkeys: true,
            renderer: { scheme: "v12code-dev", host: "app" },
          },
        ],
      ]);
  ...
  it.effect("preserves bridge initialization failures", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(error.stateDir, "/tmp/v12code-state");
      assert.equal(error.isDevelopment, true);
      assert.strictEqual(error.cause, cause);
      assert.equal(
        error.message,
        'Failed to initialize the desktop Clerk bridge for state directory "/tmp/v12code-state" (development: true).',
      );
    });
  });
  ...
  it.effect("preserves bridge cleanup failures", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(
          error.message,
          'Failed to clean up the desktop Clerk bridge for state directory "/tmp/v12code-state" (development: false).',
        );
      }
    });
  });
  ...
  it.each([
    { isDevelopment: true, scheme: "v12code-dev" },
    { isDevelopment: false, scheme: "v12code" },
    ...
    assert.equal(
      DesktopClerk.createDesktopClerkBridge("/tmp/v12code-state", isDevelopment),
      bridge,
    );
    assert.deepEqual(storageMock.mock.calls, [[{ path: "/tmp/v12code-state" }]]);
    assert.deepEqual(createClerkBridgeMock.mock.calls, [
      [
        {
          storage: storageAdapter,
