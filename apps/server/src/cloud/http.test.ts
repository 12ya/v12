...
import type { RelayLinkProofRequest } from "@v12code/contracts/relay";
import {
  ...
  it.effect("does not accept an unauthenticated request trace parent", () =>
    Effect.gen(function* () {
      ...
      const request = HttpServerRequest.fromWeb(
        new Request("https://environment.example.test/api/v12code-cloud/mint-credential", {
          headers: {
...
describe("link proof provider kinds", () => {
  ...
  it("accepts managed and manual endpoints but not v12code_relay", () => {
    ...
    expect(isSupportedLinkProviderKind(proofRequest("v12code_relay"))).toBe(false);
  });
