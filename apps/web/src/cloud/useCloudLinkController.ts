...
/**
 * Drives the primary environment's V12Code Connect link. V12Code Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update V12Code Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[v12code-connect] Could not update V12Code Connect", {
      message,
      traceId,
      cause,
    });
    ...
    toastManager.add({
      type: "error",
      title: "Could not update V12Code Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn V12Code Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to V12Code Connect before enabling this."));
        return false;
      }
