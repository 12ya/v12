...
describe("hasCloudPublicConfig", () => {
  it("requires both public cloud values", () => {
    ...
    vi.stubEnv("VITE_V12CODE_RELAY_URL", "");
    expect(hasCloudPublicConfig()).toBe(false);

    vi.stubEnv("VITE_CLERK_PUBLISHABLE_KEY", "pk_test_example");
    expect(hasCloudPublicConfig()).toBe(false);

    vi.stubEnv("VITE_CLERK_JWT_TEMPLATE", "v12code-relay");
    expect(hasCloudPublicConfig()).toBe(false);

    vi.stubEnv("VITE_V12CODE_RELAY_URL", "https://relay.example.test");
  ...
  it("rejects an insecure relay URL", () => {
    vi.stubEnv("VITE_CLERK_PUBLISHABLE_KEY", "pk_test_example");
    vi.stubEnv("VITE_CLERK_JWT_TEMPLATE", "v12code-relay");
    vi.stubEnv("VITE_V12CODE_RELAY_URL", "http://relay.example.test");
  ...
  it("reports the missing Clerk JWT template as structured configuration", () => {
    ...
    expect(() => resolveRelayClerkTokenOptions()).toThrowError(
      new CloudPublicConfigMissingError({ key: "V12CODE_CLERK_JWT_TEMPLATE" }),
    );
  });
});
