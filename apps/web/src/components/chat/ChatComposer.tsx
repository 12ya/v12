...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const pendingContextTaskCount = useTaskHudStore((state) => {
    ...
    return filterPendingContextTasks(tasks).length;
  });
  ...
  const collapsedComposerPrimaryActionDisabled =
    isConnecting || !composerSendState.hasSendableContent;
  const collapsedComposerPrimaryActionLabel =
    phase === "running"
      ? "Queue message behind the active run"
      : isSendBusy
      ...
      {isDraggingFiles
        ? createPortal(
            <div className="pointer-events-none fixed inset-3 z-[100] flex items-center justify-center rounded-2xl border-2 border-dashed border-primary/55 bg-background/80 backdrop-blur-sm [-webkit-app-region:no-drag]">
              <div className="flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2 text-sm font-medium text-foreground shadow-lg">
