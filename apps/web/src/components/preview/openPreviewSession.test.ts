import type { PreviewOpenInput, PreviewSessionSnapshot, ScopedThreadRef } from "@v12code/contracts";
...
describe("openPreviewSession", () => {
  ...
  it("applies the RPC response without waiting for a preview event", async () => {
    ...
    expect(open).toHaveBeenCalledWith({ threadId: "thread-1", url: "v12code.chat" });
    expect(readThreadPreviewState(threadRef).snapshot).toEqual(snapshot);
    expect(readThreadPreviewState(threadRef).recentlySeenUrls).toEqual(["https://v12code.chat/"]);
  });
