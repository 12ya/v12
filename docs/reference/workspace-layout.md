...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `v12` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@v12/shared/git`, `@v12/shared/DrainableWorker`) — no barrel index.
