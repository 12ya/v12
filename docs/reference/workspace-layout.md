...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `v12code` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@v12code/shared/git`, `@v12code/shared/DrainableWorker`) — no barrel index.
