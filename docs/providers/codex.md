...
This guide is for people who want to use more than one Codex account in V12Code.
...
- both accounts can see the same V12Code/Codex sessions
...
In V12Code Settings, name it something obvious:
...
In V12Code Settings, add another Codex provider:
...
V12Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, V12Code treats it
