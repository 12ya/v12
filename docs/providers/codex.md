...
This guide is for people who want to use more than one Codex account in V12.
...
- both accounts can see the same V12/Codex sessions
...
In V12 Settings, name it something obvious:
...
In V12 Settings, add another Codex provider:
...
V12 shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, V12 treats it
