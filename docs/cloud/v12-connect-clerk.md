# V12 Connect Clerk Setup

V12 Connect uses one Clerk application for web, desktop, and mobile authentication. The relay accepts
Clerk JWTs only when they are generated from the `v12-relay` template with the shared
`v12-relay` audience.

## Application Keys

V12 Connect is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:

```dotenv
V12_CLERK_PUBLISHABLE_KEY=<publishable key>
V12_CLERK_JWT_TEMPLATE=<JWT template name>
V12_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
...
should set `V12_CLERK_PUBLISHABLE_KEY`, `V12_CLERK_JWT_TEMPLATE`,
`V12_CLERK_CLI_OAUTH_CLIENT_ID`, and `V12_RELAY_URL` before building. EAS preview and
...
absent, the `v12 connect` CLI command group is omitted. The bundled server still accepts runtime
...
deployment defaults.
`vp run --filter v12-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
The `v12 connect` commands authorize a headless environment with a separate Clerk OAuth application.
This uses an OAuth public client with PKCE, so the CLI stores no client secret.

In **Clerk Dashboard > OAuth applications**:

1. Create an OAuth application for the V12 CLI.
...
5. Set `V12_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
`v12 connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `v12 connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running V12 server. The next `v12 serve` or `v12 start` reconciles the relay link and launches the
managed tunnel. `v12 connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `v12 connect link` can re-enable exposure without another browser flow. `v12 connect
...
forward that port before running `v12 connect login` or `v12 connect link`:
...
| ------- | ------------------------ |
| Name    | `v12-relay`              |
| Claims  | `{ "aud": "v12-relay" }` |

Set `V12_CLERK_JWT_TEMPLATE=v12-relay` in the repository-root `.env`, and set
`CLERK_JWT_AUDIENCE=v12-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `V12_RELAY_URL` still
...
Local desktop development uses `v12-dev://app`, while packaged builds use `v12://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `v12-dev://app`; the production Clerk instance should only need
`v12://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `com.v12.v12`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.v12.v12` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.v12.v12`.
...
When `V12_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`V12_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `V12_PORT` set. Rebuild the signed app
...
```sh
codesign --verify --deep --strict "/Applications/V12.app"
codesign -d --entitlements :- "/Applications/V12.app"
```
...
Approved signed-in users manage V12 Connect under **Connections**. The web and desktop sidebars do
not expose a dedicated account or waitlist control. Signed-out users reach Clerk's waitlist and
sign-in flow contextually from the V12 Connect controls on the Connections page.

On mobile, signed-out users open **Settings > V12 Account** to reach `/settings/waitlist` within the
