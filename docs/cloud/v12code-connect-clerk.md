# V12Code Connect Clerk Setup

V12Code Connect uses one Clerk application for web, desktop, and mobile authentication. The relay accepts
Clerk JWTs only when they are generated from the `v12code-relay` template with the shared
`v12code-relay` audience.

## Application Keys

V12Code Connect is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:

```dotenv
V12CODE_CLERK_PUBLISHABLE_KEY=<publishable key>
V12CODE_CLERK_JWT_TEMPLATE=<JWT template name>
V12CODE_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
V12CODE_RELAY_URL=https://relay.example.com
```
...
should set `V12CODE_CLERK_PUBLISHABLE_KEY`, `V12CODE_CLERK_JWT_TEMPLATE`,
`V12CODE_CLERK_CLI_OAUTH_CLIENT_ID`, and `V12CODE_RELAY_URL` before building. EAS preview and
...
absent, the `v12code connect` CLI command group is omitted. The bundled server still accepts runtime
...
deployment defaults.
`vp run --filter v12code-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
The `v12code connect` commands authorize a headless environment with a separate Clerk OAuth application.
This uses an OAuth public client with PKCE, so the CLI stores no client secret.

In **Clerk Dashboard > OAuth applications**:

1. Create an OAuth application for the V12Code CLI.
...
5. Set `V12CODE_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
```sh
v12code connect login
v12code connect link
v12code connect status
v12code connect unlink
v12code connect logout
v12code serve
```

`v12code connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `v12code connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running V12Code server. The next `v12code serve` or `v12code start` reconciles the relay link and launches the
managed tunnel. `v12code connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `v12code connect link` can re-enable exposure without another browser flow. `v12code connect
...
forward that port before running `v12code connect login` or `v12code connect link`:
...
| ------- | ---------------------------- |
| Name    | `v12code-relay`              |
| Claims  | `{ "aud": "v12code-relay" }` |

Set `V12CODE_CLERK_JWT_TEMPLATE=v12code-relay` in the repository-root `.env`, and set
`CLERK_JWT_AUDIENCE=v12code-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `V12CODE_RELAY_URL` still
...
Local desktop development uses `v12code-dev://app`, while packaged builds use `v12code://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `v12code-dev://app`; the production Clerk instance should only need
`v12code://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `com.v12code.v12code`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.v12code.v12code` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.v12code.v12code`.
...
V12CODE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/v12code.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
V12CODE_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `V12CODE_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`V12CODE_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `V12CODE_PORT` set. Rebuild the signed app
...
```sh
codesign --verify --deep --strict "/Applications/V12Code.app"
codesign -d --entitlements :- "/Applications/V12Code.app"
```
...
Approved signed-in users manage V12Code Connect under **Connections**. The web and desktop sidebars do
not expose a dedicated account or waitlist control. Signed-out users reach Clerk's waitlist and
sign-in flow contextually from the V12Code Connect controls on the Connections page.

On mobile, signed-out users open **Settings > V12Code Account** to reach `/settings/waitlist` within the
