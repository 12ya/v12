...
The screenshot harness runs the real mobile application against three disposable local V12Code
...
the software keyboard, and supplies deterministic V12Code Connect discovery rows to the real
...
1. Creates three temporary V12Code base directories and starts a local server for each on an available
   port.
2. Creates V12Code, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic V12Code review diff.
...
ARM64 by default for local Apple Silicon development; CI sets `V12CODE_SHOWCASE_ANDROID_ABI=x86_64` so the
...
Run Metro separately:
    ...
    pnpm --filter @v12code/mobile showcase
...
The Pending rows use the production offline outbox and point at the real V12Code and React fixture
