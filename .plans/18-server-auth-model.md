...
Define the long-term server auth architecture for V12Code before first-class remote environments ship.
...
Every privileged surface of the V12Code server must go through the same auth policy engine:
...
```text
Participants:
  UserShell     = npx v12code launcher
...
DesktopMain -> Tunnel : publish local V12Code endpoint
...
Participants:
  ...
  RemoteT3      = remote V12Code server
  Frontend      = desktop renderer

DesktopUser -> DesktopMain : add SSH host
DesktopMain -> SSH : connect to remote host
SSH -> RemoteHost : probe environment / verify v12code availability
DesktopMain -> SSH : run remote launch command
SSH -> RemoteHost : v12code remote launch --json
